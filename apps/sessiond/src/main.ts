...
export async function runSessiond(): Promise<{
  ...
  try {
    ...
    const database = new SessionDatabase(paths.databaseFile);
    db = database;
    const interrupted = database.sessions.markOrphansInterrupted();
    const interruptedRuns = database.schedules.markOrphanedRunsFailed();
    const notifications = new NotificationCenter(database.notifications);
    supervisor = new SessionSupervisor(
      database.sessions,
      (operation) => database.transaction(operation),
      (...args) => database.audit.write(...args),
      config,
      paths,
      notifications
    );
    const piManager = new PiManager(config, database.audit);
    ...
    const sessionFolders = new SessionFolderStore(
      database.settings,
      database.sessions,
      (operation) => database.transaction(operation),
      database.audit,
    ...
    ipc = new IpcServer({
      ...
      notificationStore: database.notifications,
