...
export class SessionFolderStore {
  ...
  constructor(db: SessionDatabase, paths: PiWebPaths) {
    this.#db = db;
  ...
  async initialize(): Promise<void> {
    try {
      if (this.#db.getSetting<unknown>(sessionFolderSettingKey) !== null) {
        this.#read();
        return;
      }
    } catch {
    ...
    try {
      ...
      this.#db.audit("session_folders.migrate", "success", "system", null, {
  ...
  #read(): SessionFolderState {
    try {
      const value = this.#db.getSetting<unknown>(sessionFolderSettingKey);
      if (value === null) {
  ...
  #write(state: SessionFolderState): void {
    this.#db.setSetting(sessionFolderSettingKey, this.#parseState(state));
