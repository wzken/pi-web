...
import { t } from "../../../i18n";
import { dismissTopMobileLayer } from "../../../mobile-back-stack";
import { useNavigate } from "../../../router";
import { Composer } from "./Composer";
import { ExtensionInteractionDialog } from "./ExtensionInteractionDialog";
...
export function SessionDetailView({
  ...
  useLayoutEffect(() => {
    ...
    const marker = `pi-web-session-back:${sessionId}`;
    window.history.pushState(
      { ...(window.history.state ?? {}), piWebMobileBackMarker: marker },
      "",
    ...
    const onBack = () => {
      if (dismissTopMobileLayer()) {
        window.queueMicrotask(rearm);
        return;
      }
      const layers = mobileLayers.current;
      if (layers.filesOpen) setFilesOpen(false);
      else if (layers.terminalOpen) {
        setTerminalCollapsed(false);
        setTerminalOpen(false);
      } else if (layers.systemOpen) setSystemOpen(false);
      else if (layers.treeOpen) setTreeOpen(false);
      else if (layers.railOpen) setRailOpen(false);
      else {
  ...
  }, [navigate, sessionId, setRailOpen]);
  ...
  return (
    ...
    >
      ...
      <ExtensionInteractionDialog
        sessionId={sessionId}
        interaction={snapshot.pendingInteractions[0] ?? null}
        onError={onError}
        onResolved={onRuntimeUpdated}
      />
    </div>
  );
}
