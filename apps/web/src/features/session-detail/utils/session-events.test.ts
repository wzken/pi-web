...
describe("session events", () => {
  ...
  it("applies model, thinking, and pin events to the session projection", () => {
    const modeled = applyRealtimeEvent(
      makeState(),
      makeEvent("session.model_changed", {
        model: "openai/gpt-5",
        thinkingLevel: "high"
      })
    );
    const thinking = applyRealtimeEvent(
      modeled,
      makeEvent("session.thinking_changed", { thinkingLevel: "low" }, 6)
    );
    const pinned = applyRealtimeEvent(
      thinking,
      makeEvent("session.pinned", { pinned: true }, 7)
    );

    expect(pinned.snapshot?.session).toMatchObject({
      model: "openai/gpt-5",
      thinkingLevel: "low",
      pinned: true
    });
    expect(pinned.snapshot?.sequence).toBe(7);
  });
