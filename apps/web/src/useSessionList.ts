import type { SessionListPage, SessionRecord } from "@pi-web/protocol";
import {
  useCallback,
  useEffect,
  useRef,
...
export function useSessionList(): SessionListState {
  ...
  const refresh = useCallback(async (force = false) => {
    ...
    try {
      const next = await fetchAllSessions(controller.signal);
      if (
...
export async function fetchAllSessions(
  ...
  do {
    ...
    const page = await api<SessionListPage>(`/api/sessions?${query}`, {
