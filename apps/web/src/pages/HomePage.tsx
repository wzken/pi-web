...
export function HomePage() {
  ...
  }, [files.length, images.length, mobileViewport, prompt]);
      ...
      <main className={ui("home-start")}>
        ...
        <div className={ui("home-start-content")}>
          ...
          <div className={ui("home-start-inner")}>
            ...
            <form
              className={ui(
                `home-composer${dragActive ? " attachment-drag-active" : ""}`
              )}
            ...
            >
              ...
              <div className={ui("home-composer-context")}>
                <button
                  type="button"
                  className={ui("home-current-workspace")}
                  onClick={() => setRuntimeOpen(true)}
                  aria-label={t("选择工作目录")}
                >
                  <span>{t("工作目录")}</span>
                  <strong>{cwd ? folderName(cwd) : t("选择工作目录")}</strong>
              ...
              <div className={ui("home-composer-bar")}>
                <div className={ui("home-composer-tools")}>
                  ...
                  >
                    ...
                    <span>{t("运行设置")}</span>
                    <small>
                      {shortModelName(model) || t("默认模型")} · {thinkingLevel || t("默认思考")}
                    </small>
                    <ChevronDown size={13} />
                  </Button>
                  {systemPrompt.trim() ? (
