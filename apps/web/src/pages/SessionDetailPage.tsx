...
import { Button, ButtonLink, ErrorBanner, Loading, useToast } from "../components";
...
export function SessionDetailPage() {
  ...
  const handleRealtimeEvent = useCallback(
    (event: RealtimeEvent) => {
      ...
      if (event.type === "extension_ui.notification") {
        const payload =
          event.payload && typeof event.payload === "object"
            ? (event.payload as Record<string, unknown>)
            : {};
        if (payload.method === "notify" && typeof payload.message === "string") {
          toast.push(
            payload.message,
            payload.notifyType === "error" ? "error" : "success"
          );
        }
      }
      return disposition;
    },
    [acceptRealtimeEvent, requestSnapshotReconciliation, toast]
  );
  ...
  useEffect(() => {
    function clearWhenVisible() {
      if (document.visibilityState === "visible") {
        void markSessionRead(id).catch(() => undefined);
      }
    }
    clearWhenVisible();
