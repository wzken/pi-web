...
export function registerSessionRoutes(
  ...
  app.get<{ Querystring: { limit?: string; cursor?: string } }>(
    ...
    async (request) => {
      ...
      return await client.request("sessions.list", input);
    }
  );
  ...
  app.post<{ Params: { id: string }; Body: unknown }>(
    "/api/sessions/:id/extension-ui",
    async (request) =>
      await client.request("sessions.extension_ui_response", {
        id: idSchema.parse(request.params.id),
        ...extensionUiResponseSchema.parse(request.body)
      })
  );
  app.post<{ Params: { id: string } }>(
