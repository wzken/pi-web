...
describe("session workspace authorization", () => {
  it("lists workspace files and reads text for a session file browser", async () => {
    const cwd = await mkdtemp(join(tmpdir(), "pi-web-session-files-"));
    await writeFile(join(cwd, "README.md"), "# Workspace\n");

    const list = await listWorkspaceEntries(cwd);
    ...
    expect(await readWorkspaceText(cwd, "README.md")).toEqual({
      path: "README.md",
