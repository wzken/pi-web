...
- Reconnect through bounded event replay using `projectionEpoch + sequence` or
  a fresh Pi JSONL snapshot.
- Stable Pi entry-ID pagination over the complete active branch, with durable
  compaction markers kept separate from `retainedTail` current-context data.
- Pi Extension UI support for select, confirm, input, editor, and notifications;
  pending dialogs survive browser refreshes.
- A persistent supervision inbox for pending decisions, completed turns, worker
  failures, and failed schedules, with unread and needs-attention views shared
  across signed-in browsers.
- Standards-based Web Push with server-owned VAPID keys, per-device
  subscriptions, bounded retries, expired-subscription cleanup, and deep links
  back to the relevant session. Lock-screen payloads contain only the event
  type and session or schedule name.
- Remote-access discovery for the current origin, private LAN addresses, and an
  existing Tailscale HTTPS Serve endpoint, with an offline-generated QR code
  and exposure warnings. Pi Web never changes Tailscale or firewall state.
...
- Responsive desktop/mobile UI, command palette, persistent notification
  inbox, background Push, remote-access QR code, and PWA assets.
...
The compatibility baseline is Pi Coding Agent 0.84.1. Before the first real
worker starts, Pi Web performs a no-model RPC capability probe from a neutral
directory. Unit, integration, and E2E tests still use a process-level fake Pi
worker and require no provider credentials.
...
5. Use the notification center to handle Extension UI decisions, completed
   turns, worker failures, and failed schedules. On an HTTPS or loopback origin,
   enable background Push for alerts after the browser closes.
6. Create a schedule when a prompt needs to run later or repeatedly; each run
...
| Path | Responsibility |
| --- | --- |
| `apps/web` | React/Vite PWA, session workbench, notification inbox, Push subscription UI, settings, files, terminal, and schedules |
...
| `stop` / `restart` | Stop or restart Linux user services; active workers require an explicit `--force` interruption |
| `doctor` | Check Node, Pi 0.84.1+ compatibility, required RPC commands, SQLite, IPC, roots, server health, and exposure |
...
| `uninstall` | Remove systemd units while preserving configuration and data; active workers require `--force` |
...
network, an SSH tunnel, an existing Tailscale Serve HTTPS endpoint, or an HTTPS
reverse proxy. The Settings page detects available private addresses and
renders a QR code, but it never changes network exposure automatically.

Background Push requires a secure browser context: HTTPS, or loopback during
local development. VAPID private keys and Push subscriptions are stored in the
sessiond-owned SQLite database. Resetting the access key revokes all Push
subscriptions; signing out revokes the current device. Notification payloads
must not include prompts, tool output, paths, credentials, or error details.
...
- Restarting sessiond or the host interrupts active workers; Pi Web does not
  promise lossless mid-tool recovery.
- `pi-web stop`, `restart`, `uninstall`, and Pi Package changes refuse active
  workers by default. Forced operations require an explicit confirmation and
  are audited.
...
- Authenticated session data is not available offline; the PWA caches only
  compiled static assets. Its Service Worker handles authenticated Web Push
  deep links but does not cache notification or conversation records.
...
After v0.1 reliability: Git status and diff review, worktree-aware workflows,
message deep links and forks, richer tool output review, and deeper long-session
performance benchmarks. Multi-user, fleet, SaaS, and generic multi-agent
orchestration features remain out of scope.
